...
**Note:** The application is configured for both SQLite (currently active) and PostgreSQL (via Drizzle schema), allowing for future database migration if needed. The storage layer uses an interface pattern that abstracts database operations, making it straightforward to swap implementations.